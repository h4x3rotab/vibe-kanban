...
pub struct ExecutionProcessHistoryQuery {
    ...
    pub skip_tail_entries: Option<usize>,
}
...
async fn stream_raw_logs_ws(
    ...
    Query(query): Query<ExecutionProcessHistoryQuery>,
) -> impl IntoResponse {
    let tail_entries = query.tail_entries.or_else(chat_history_tail_entries);
    let skip_tail_entries = query.skip_tail_entries;
    ...
    ws.on_upgrade(move |socket| async move {
        if let Err(e) =
            handle_raw_logs_ws(socket, deployment, exec_id, tail_entries, skip_tail_entries).await
        {
...
async fn handle_raw_logs_ws(
    ...
    let raw_stream = match deployment
        .container()
        .stream_raw_logs(&exec_id, tail_entries, skip_tail_entries)
        .await
    {
        Some(stream) => stream,
        None => {
...
async fn stream_normalized_logs_ws(
    ...
    ws.on_upgrade(move |socket| async move {
        let stream = deployment
            .container()
            .stream_normalized_logs(&exec_id, tail_entries, skip_tail_entries)
            .await;
        match stream {
            Some(stream) => {
